...
class NewClaimAnalyzer:
    ...
    def store_metadata_in_qdrant(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        Store claim analysis metadata in Qdrant with better error handling
        """
        try:
            # Generate unique ID for this analysis
            vector_id = str(uuid.uuid4())
            
            # Create vector embedding from metadata
            embedding = self._create_embedding_from_metadata(metadata)
            
            print(f"Created embedding with {len(embedding)} dimensions")
            print(f"Embedding sample: {embedding[:10]}...")  # Print first 10 values
            
            # Prepare payload (ensure all values are JSON serializable)
            payload = {
                "claim_id": str(metadata.get("claim_id", "unknown")),
                "patient_id": str(metadata.get("patient_id", "unknown")),
                "provider_id": str(metadata.get("provider_id", "unknown")),
                "service_date": str(metadata.get("service_date", "")),
                ...
                "cpt_codes": list(metadata.get("all_cpt_codes", [])),
                "icd_codes": list(metadata.get("icd_codes", [])),
                # Store simplified metadata to avoid serialization issues
                "risk_summary": {
                    "max_risk": float(metadata.get("max_risk_score", 0)),
                    "critical_count": int(metadata.get("critical_issues", 0)),
                    "high_count": int(metadata.get("high_issues", 0))
                }
            }
            
            print(f"Payload prepared: {list(payload.keys())}")
            ...
            print(f"Upsert operation result: {operation_result}")
    ...
    def _create_embedding_from_metadata(self, metadata: Dict[str, Any]) -> List[float]:
        ...
        if len(features) < target_size:
            ...
            padding = np.random.normal(0, 0.01, remaining).tolist()
    ...
    def test_qdrant_insertion(self):
        """Test Qdrant insertion with simple data"""
        try:
            ...
            test_metadata = {
                ...
                "critical_issues": 1,
                "high_issues": 1,
                "max_risk_score": 120.5,
                "avg_risk_score": 85.2,
                "all_cpt_codes": ["99213", "80053"],
                "icd_codes": ["25000", "4019"],
                "risk_metadata": {"high_risk_issues": 2},
                "issue_categories": {"duplicate_billing": ["99213"]},
                "denial_indicators": {"high_denial_risk": True},
                "provider_type": "Physician",
                "procedure_categories": {"evaluation_management": ["99213"], "laboratory": ["80053"]},
                "diagnosis_categories": {"endocrine": ["25000"], "cardiovascular": ["4019"]},
                "billing_patterns": {"duplicate_procedures": [], "high_cost_procedures": []},
                "units": {"99213": 1, "80053": 1},
                "modifiers": []
            }
            
            print("Testing Qdrant insertion...")
            result = self.store_metadata_in_qdrant(test_metadata)
    ...
    def _determine_lcd_coverage(self, conn, hcpcs_code, diagnosis_code):
        ...
        Backup saved as: new_claim_analyzer.py.backup_before_article_db_YYYYMMDD_HHMMSS
        ...
        try:
            # Strategy 1: Check LCD crosswalk in _article_ database for ALL codes (CPT + HCPCS)
            if hcpcs_code and hcpcs_code[0].isdigit():  # Numeric CPT codes
                try:
                    query = """
                        SELECT TOP 1 coverage_status
                        FROM [_article_].[dbo].[vw_LCD_CPT_HCPCS_Crosswalk]
                        WHERE hcpcs_cpt_code = ?
                          AND icd10_code = ?
...
if __name__ == "__main__":
    ...
    has_points = analyzer.check_qdrant_status()
    
    if not has_points:
        print("No points found. Testing insertion...")
        test_result = analyzer.test_qdrant_insertion()
    ...
    if 'metadata' in result:
        print("\n=== Storing in Qdrant ===")
        qdrant_status = analyzer.store_metadata_in_qdrant(result['metadata'])
        print("Qdrant storage status:", qdrant_status)
        
        # Verify storage
        print("\n=== Verifying Storage ===")
        analyzer.list_all_points()
