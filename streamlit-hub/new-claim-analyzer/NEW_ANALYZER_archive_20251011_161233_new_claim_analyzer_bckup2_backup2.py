...
class NewClaimAnalyzer:
    ...
    def store_metadata_in_qdrant(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        Store claim analysis metadata in Qdrant
        ...
        try:
            ...
            payload = {
                ...
                "max_risk_score": metadata.get("max_risk_score", 0),
                "avg_risk_score": metadata.get("avg_risk_score", 0),
                "cpt_codes": metadata.get("all_cpt_codes", []),
    ...
    def _create_embedding_from_metadata(self, metadata: Dict[str, Any]) -> List[float]:
        """
        Create a vector embedding from metadata
        ...
        try:
            ...
            for dx_pos, dx_code in dx_codes:
                for hcpcs_pos, hcpcs_code in hcpcs_codes:
                    ...
                    lcd_covered = self._determine_lcd_coverage(mapped_icd10 if mapped_icd10 else dx_code)
    ...
    def _determine_lcd_coverage(self, diagnosis_code):
        """Determine LCD coverage for diagnosis code (ICD-9 or ICD-10)"""
        if not diagnosis_code:
            return 'N'
        
        # Enhanced coverage logic for both ICD-9 and ICD-10
        covered_codes = [
            # ICD-9 codes
