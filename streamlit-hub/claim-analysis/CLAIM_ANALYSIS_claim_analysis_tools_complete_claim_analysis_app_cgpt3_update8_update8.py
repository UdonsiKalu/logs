#!/usr/bin/env python3
"""
CXR Exchange — Final Unified Version (UPDATE8)
--------------------------------------
...
UPDATE8 Changes:
- Real-time terminal log display during processing
- Removed emojis from all outputs
- Updated dropdown label to "Archetype-Driven"
"""
...
# Commented out broken imports - only using UPDATE10
# from claim_corrector_claims3_archetype_driven_v2 import ArchetypeDrivenClaimCorrectorV2
from claim_corrector_claims3_archetype_driven_update10 import ArchetypeDrivenClaimCorrector as ArchetypeDrivenClaimCorrectorV3
...
# Runner wrappers (only working modes)
...
# Broken modes commented out (calibrated versions have syntax errors)
    ...
    with st.expander("Stage 3 – Correction Recommendations", expanded=False):
        ...
        if rec_cards:
            for r in rec_cards:
                ...
                #  FIX: LLM returns 'suggestion' not 'suggested_value'
                suggested_val = r.get('suggestion', r.get('suggested_value', ''))
                rationale = r.get('rationale', '')
                policy_cite = r.get('policy_citation', r.get('policy_reference', ''))
                #  FIX: LLM returns 'implementation_guidance' not 'implementation_steps'
                impl_steps = r.get('implementation_guidance', r.get('implementation_steps', []))
...
def main():
    ...
    with st.sidebar:
        ...
        mode = st.selectbox(
            ...
            index=2,  # Default to Archetype-Driven with sub-archetypes
        )
    ...
    if claim:
        ...
        if st.button("Run Analysis"):
            ...
            try:
                ...
                if final_log:
                    log_display.code(final_log, language='text')
                
                log_placeholder.success("Analysis complete")
                
                # Display results
                if mode == "Archetype-Driven" and isinstance(out, dict):
                    show_v3_summary(out)
                else:
                    st.json(out)
