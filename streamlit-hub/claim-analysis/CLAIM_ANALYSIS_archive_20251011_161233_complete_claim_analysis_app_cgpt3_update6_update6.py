...
# Commented out broken imports - only using UPDATE6
# from claim_corrector_claims3_archetype_driven_v2 import ArchetypeDrivenClaimCorrectorV2
from claim_corrector_claims3_archetype_driven_update6 import ArchetypeDrivenClaimCorrector as ArchetypeDrivenClaimCorrectorV3
    ...
    with st.sidebar:
        ...
        mode = st.selectbox(
            ...
            [
                "Basic Analysis",
                "Two-Stage Pipeline",
                "Archetype-Driven (UPDATE6)",
            ],
            index=2,  # Default to UPDATE6
        )
        ...
        if st.button("Run Analysis"):
            with st.spinner(f"Running {mode}..."):
                try:
                    ...
                    elif mode == "Archetype-Driven (UPDATE6)":
                        _ = run_basic(claim)
                    ...
                    if mode.endswith("(UPDATE6)") and isinstance(out, dict):
                        show_v3_summary(out)
                    else:
                        st.json(out)
                except Exception as e:
