...
from claim_corrector_claims3_archetype_driven_update4 import ArchetypeDrivenClaimCorrector as ArchetypeDrivenClaimCorrectorV3
        ...
        if st.button("Run Analysis"):
            with st.spinner(f"Running {mode}..."):
                try:
                    ...
                    if mode.endswith("(UPDATE4)") and isinstance(out, dict):
                        show_v3_summary(out)
                    else:
                        st.json(out)
                except Exception as e:
