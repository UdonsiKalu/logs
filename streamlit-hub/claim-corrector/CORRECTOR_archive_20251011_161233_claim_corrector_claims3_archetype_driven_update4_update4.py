    ...
    def _get_icd10_alternatives_from_db(self, icd10_code: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query_shared_icd9 = """
                ...
                SELECT TOP ? a.icd10_code, a.icd9_code, c.description
                FROM alternatives a
                LEFT JOIN [_gems].[dbo].[icd10cm_codes_2018_fixed] c 
                    ON a.icd10_code = c.icd10_code
                ORDER BY a.icd10_code
            """
            df_shared = pd.read_sql(query_shared_icd9, self.connection, params=[icd10_code, icd10_code, limit])
            ...
            # Extract category (first 3-4 chars depending on pattern)
            if len(icd10_code) >= 4 and icd10_code[3] in ['.', '-']:
                pattern = icd10_code[:3] + '%'
            elif len(icd10_code) >= 4:
                pattern = icd10_code[:4] + '%'
            else:
                pattern = icd10_code[:3] + '%'
            
            query_pattern = """
                SELECT TOP ? g.icd10_code, c.description
                FROM [_gems].[dbo].[table_2018_I9gem_fixed] g
                LEFT JOIN [_gems].[dbo].[icd10cm_codes_2018_fixed] c
                    ON g.icd10_code = c.icd10_code
                WHERE g.icd10_code LIKE ?
                  AND g.icd10_code != ?
                ORDER BY g.icd10_code
            """
            df_pattern = pd.read_sql(query_pattern, self.connection, params=[limit, pattern, icd10_code])
            ...
            if not df_pattern.empty:
                ...
                print(f"    Found {len(alternatives)} alternatives via pattern matching ({pattern})")
                return alternatives
